    ...
    function changeInfo (res) {
        ...
        for (var i = start; i < end; i++) {
            ...
            html += '<div class="department-info">' +
                ...
                '<div class="seeing info-item">' + SliceName(res.list[i].listInfo.seeing.name)+ ReturnStatus(res.list[i].listInfo.seeing.status,res.list[i].listInfo.seeing.id) + '</div>' +
                '<div class="waiting info-item">';

            for (var j = 0; j < waitingNum; j++) {
                html += '<div class="waiting-item">' + SliceName(res.list[i].listInfo.waiting[j].name)+ ReturnStatus(res.list[i].listInfo.waiting[j].status) +'</div>'
            }
            if (waitingNum < 2){
                for (var i = 0; i < 2 - waitingNum; i++) {
